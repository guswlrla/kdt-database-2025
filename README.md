...
### 1. 서브쿼리
- 하나의 sql문 안에 다른 sql문이 중첩된 질의, 조건 필터링이나 집계 등에 사용
- 대량 데이터에서는 조인보다 성능 효율이 좋을 수 있음

1) 중첩질의(WHERE 부속질의)
    - WHERE절 안에서 다른 쿼리 결과를 조건으로 사용
    - 비교 연산자(`=`, `>`, `<`, `>=`, `<=`, `<>`)는 단일행 반환
    -  (`IN`, `NOT IN`)연산자는 다중행 반환
    -  존재 연산자(`EXISTS`, `NOT EXISTS`)는 서브쿼리 결과 존재 여부 확인, TRUE/FALSE 반환

2) 스칼라 부속질의(SELECT 부속질의)
    - SELECT 절 안에서 하나의 값을 반환하는 서브쿼리 사용

3) 인라인 뷰(FROM 부속질의)
    - FROM절 안에서 서브쿼리를 테이블처럼 사용

### 2. 집합 연산자
- 두 개 이상의 테이블에서 SELECT문의 결과를 대상으로 연산을 수행하는 연산자
   - `UNION` : 두 개의 테이블을 하나로 만드는 연산, **중복된 데이터는 제거**
   - `UNION ALL` : UNION과 거의 같은 기능을 수행하되 **중복 포함**
   - `INTERSECT` : 두 개의 테이블에 대해 겹치는 부분을 추출, MySQL에서는 지원되지 않음
   - `EXCEPT(MINUS)` : 두 개의 테이블에서 겹치는 부분을 앞의 테이블에서 제외해서 추출, MySQL에서는 지원되지 않음
